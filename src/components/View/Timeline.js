...
export default class TimeLine extends Component {
	...
	componentDidMount() {
		...
		if(Platform.OS == 'ios') {
			this.refs.scrollView.scrollTo(0, (this.state.selected.index - 1) * 120);
	...
	moveTo(date) {
		var dateObject = { date: moment(date).format(dateFormat) };
	...
	_updateFrame(dateTime) {
		...
		if(this.refs.scrollView)
			this.refs.scrollView.scrollTo(0, (dateTime.index - 1) * 120);
		...
		if(this.state.selected.date == dateObject.date) {
			return (
				<TouchableOpacity
					...
					<Image source={assets.active_arrow} style={styles.arrow} />
				</TouchableOpacity>
			)
		}else {
			return (
