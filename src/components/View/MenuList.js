...
var MenuList = React.createClass({
  ...
  renderlHeader: function(){
    ...
    for(var i in data){
      ...
      header.push(
        <TouchableOpacity key={"Header_"+i} style={[styles.flex_1, styles.center]}
    ...
    for(var i in data){
      if(index === tabSelected){
        for(var k in data[i]){
          ...
          leftPannel.push(
            <Text key={"Left_"+i+k} onPress={this.leftPress.bind(this, i, k)}
        ...
        for(var k in data[i]){
          if(this.state[prefixType + i + '_' + k]){
            for(var j in data[i][k]){
              rightPannel.push(
                <Text key={"Right_"+k+j+i} onPress={this.props.click.bind(this, data[i][k][j])} style={styles.left_row}>{data[i][k][j]}</Text>
              );
            }
            break;
          }
